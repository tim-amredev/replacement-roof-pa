document.addEventListener("DOMContentLoaded", () => {
  ...
  // Remove regional multipliers and use a standard multiplier for all of PA
  const regionMultipliers = {
    Pennsylvania: { min: 1.0, max: 1.0 },
  }
      ...
      requiredFields.forEach((field) => {
        if (field.type === "radio") {
          ...
          if (!checked) {
            ...
            const label = currentStepElement
  ...
  // Update the calculateEstimate function to use a standard regional multiplier
  function calculateEstimate() {
    ...
    const baseMaterialCost = materialBasePrices[userSelections.material] || materialBasePrices.asphalt
    ...
    // Apply standard regional multiplier for all of PA
    ...
    // Update the displayed estimate if the element exists
    const displayedEstimate = document.getElementById("displayed-estimate-range")
    if (displayedEstimate) {
      displayedEstimate.textContent = `$${roundedTotalCost.min.toLocaleString()} - $${roundedTotalCost.max.toLocaleString()}`
    }
